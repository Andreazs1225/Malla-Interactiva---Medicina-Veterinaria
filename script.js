...
// Crear botones y contenedor para ellos
...
botonesContainer.style.marginBottom = "1rem";
botonesContainer.style.display = "flex";
botonesContainer.style.gap = "10px";
botonesContainer.appendChild(guardarBtn);
botonesContainer.appendChild(cargarBtn);

mallaEl.parentNode.insertBefore(botonesContainer, mallaEl);
...
function refrescarBloqueos() {
  cursos.forEach(curso => {
    ...
    if (!card) return; // protección si no existe
    ...
    if (!desbloqueado && !aprobado) {
      ...
      if (!aprobado) card.querySelector("input").disabled = true;
    }
  });
}
